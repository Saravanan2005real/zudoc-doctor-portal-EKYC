...
def window_partiton(x,window_size:int):# Patch_Partition
    ...
    # permute: [B, H//Mh, Mh, W//Mw, Mw, C] -> [B, H//Mh, W//Mh, Mw, Mw, C]   转置
    ...
    def __init__(self, dim, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                shift_size=0 if (i % 2 == 0) else self.shift_size,   #i表示层数，SwinTransformerBlock是成对使用
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                drop=drop,
                attn_drop=attn_drop,
...
print("Net Number of parameter: %.2fM" % (p_num / 1e6))
x = torch.randn(1,3,224,224)
print(net(x).shape)
...
print("Net Number of parameter: %.2fM" % (params / 1e6))
print("Net Number of FLOPs: %.2fG" % (flops / 1e9))
